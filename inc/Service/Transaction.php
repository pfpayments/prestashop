...
class PostFinanceCheckoutServiceTransaction extends PostFinanceCheckoutServiceAbstract
{
    ...
    private const POSSIBLE_PAYMENT_METHOD_SESSION_KEY = 'pfcPossiblePaymentMethods';
     ...
     *
     * @var \PostFinanceCheckout\Sdk\Model\ChargeAttempt[]
     */
    ...
    {
        if ($property === null) {
            $property = new $className(PostFinanceCheckoutHelper::getApiClient());
        }
        return $property;
    }
    ...
    protected function getTransactionService()
    {
        return $this->getSdkService(
            ...
            \PostFinanceCheckout\Sdk\Service\TransactionService::class
        );
    }
    ...
    protected function getTransactionIframeService()
    {
        return $this->getSdkService(
            ...
            \PostFinanceCheckout\Sdk\Service\TransactionIframeService::class
        );
    }
    ...
    protected function getTransactionPaymentPageService()
    {
        return $this->getSdkService(
            ...
            \PostFinanceCheckout\Sdk\Service\TransactionPaymentPageService::class
        );
    }
    ...
    protected function getChargeAttemptService()
    {
        return $this->getSdkService(
            ...
            \PostFinanceCheckout\Sdk\Service\ChargeAttemptService::class
        );
    }
    ...
    public function getJavascriptUrl(Cart $cart)
    {
        ...
        $url = $js . '&className=postfinancecheckoutIFrameCheckoutHandler';
     ...
     * @param int $spaceId
     * @param int $transactionId
     * @return \PostFinanceCheckout\Sdk\Model\ChargeAttempt|null
     */
    ...
    public function updateTransactionInfo(\PostFinanceCheckout\Sdk\Model\Transaction $transaction, Order $order)
    {
        ...
        if (
            $transaction->getState() == \PostFinanceCheckout\Sdk\Model\TransactionState::FAILED
        ...
        if (!isset($this->chargeAttemptCache[$key])) {
            ...
            $filter->setChildren(
                ...
                $this->createEntityFilter(
                    'state',
                    \PostFinanceCheckout\Sdk\Model\ChargeAttemptState::SUCCESSFUL
                ),
                )
            );
            $query->setFilter($filter);
    ...
    public function getPossiblePaymentMethods(
        Cart $cart,
        \PostFinanceCheckout\Sdk\Model\Transaction $transaction = null
    ) {
     ...
     *
     * @param Cart $cart
     * @param \PostFinanceCheckout\Sdk\Model\Transaction|null $transaction
    ...
    private function warmPossiblePaymentMethodCache(
        ...
        $cached = PostFinanceCheckoutHelper::getCartMeta($cart, self::POSSIBLE_PAYMENT_METHOD_CACHE_KEY);
        ...
        try {
            ...
            PostFinanceCheckoutHelper::clearCartMeta($cart, self::POSSIBLE_PAYMENT_METHOD_CACHE_KEY);
            $this->clearSessionPaymentMethodCacheEntry($currentCartId);
            if ($failSilently) {
                return array();
            }
    ...
    private function getSessionPaymentMethodCacheData()
    {
        ...
        $decoded = PostFinanceCheckoutTools::base64Decode($cookie->$key);
    ...
    private function persistSessionPaymentMethodCacheData(array $data)
    {
        ...
        if (empty($data)) {
            ...
            $cookie->$key = PostFinanceCheckoutTools::base64Encode(serialize($data));
        }
        $cookie->write();
    }
    ...
    private function convertPaymentMethodsForSession(array $paymentMethods)
    {
        ...
        foreach ($paymentMethods as $method) {
            if ($method instanceof \PostFinanceCheckout\Sdk\Model\PaymentMethodConfiguration) {
    ...
    {
        $method = new \PostFinanceCheckout\Sdk\Model\PaymentMethodConfiguration();
    ...
    public function invalidateTransactionCache(Cart $cart)
    {
        ...
        PostFinanceCheckoutHelper::clearCartMeta($cart, self::CART_HASH_META_KEY);
        PostFinanceCheckoutHelper::clearCartMeta($cart, self::POSSIBLE_PAYMENT_METHOD_CACHE_KEY);
    ...
    {
        $cached = PostFinanceCheckoutHelper::getCartMeta($cart, self::TRANSACTION_CACHE_META_KEY);
        if (!is_array($cached)
        ...
        $serialized = PostFinanceCheckoutTools::base64Decode($cached['data']);
     ...
     * @return void
     */
    private function storeCachedTransactionForCart(Cart $cart, \PostFinanceCheckout\Sdk\Model\Transaction $transaction)
    {
        ...
        PostFinanceCheckoutHelper::updateCartMeta(
            ...
            array(
                ...
                'data' => PostFinanceCheckoutTools::base64Encode(serialize($transaction))
            )
        );
    }
    ...
    {
        PostFinanceCheckoutHelper::clearCartMeta($cart, self::TRANSACTION_CACHE_META_KEY);
    }
     ...
     * @return void
     */
    private function cacheLoadedTransactionObject(\PostFinanceCheckout\Sdk\Model\Transaction $transaction)
    {
        if ($transaction == null) {
            return;
        }
     ...
     * @return string|null
     */
    private function getCachedJavascriptUrl(Cart $cart, \PostFinanceCheckout\Sdk\Model\Transaction $transaction)
    {
        $cached = PostFinanceCheckoutHelper::getCartMeta($cart, self::JS_URL_CACHE_META_KEY);
        if (!is_array($cached)
    ...
    {
        PostFinanceCheckoutHelper::clearCartMeta($cart, self::JS_URL_CACHE_META_KEY);
    }
    ...
    {
        $hash = PostFinanceCheckoutHelper::getCartMeta($cart, self::CART_HASH_META_KEY);
    ...
    {
        PostFinanceCheckoutHelper::updateCartMeta($cart, self::CART_HASH_META_KEY, $cartHash);
    }
    ...
    protected function createTransactionFromCart(Cart $cart)
    {
        ...
        $spaceViewId = Configuration::get(
            PostFinanceCheckoutBasemodule::CK_SPACE_VIEW_ID,
            null,
            null,
            $cart->id_shop
        );
        ...
        $this->storeTransactionCartHash($cart, PostFinanceCheckoutHelper::calculateCartHash($cart));
        $this->clearCachedJavascriptUrl($cart);
    ...
    protected function loadAndUpdateTransactionFromCart(Cart $cart)
    {
        ...
        $transaction = $this->getTransaction($ids['spaceId'], $ids['transactionId']);
        if ($transaction === null || $transaction->getState() !== \PostFinanceCheckout\Sdk\Model\TransactionState::PENDING) {
            $transaction = $this->createTransactionFromCart($cart);
        }

        $customerId   = $transaction->getCustomerId();
        $cartCurrency = PostFinanceCheckoutHelper::convertCurrencyIdToCode($cart->id_currency);
        $cartHash     = PostFinanceCheckoutHelper::calculateCartHash($cart);
    ...
    protected function updateTransactionFromCart(
        ...
        $cartHash = $cartHash ?: PostFinanceCheckoutHelper::calculateCartHash($cart);
